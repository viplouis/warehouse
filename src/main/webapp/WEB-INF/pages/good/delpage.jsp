...
<title>商品出库</title>
<meta name="description" content="这是一个商品出库页面">
...
<script type="text/javascript">	
		/* 
				$(function() {
					var categories = $("#goodname1").kendoComboBox({  
						            ...
						            dataSource: {  
						                type: "json",  
						                transport: {  
						                    read: "${pageContext.request.contextPath }/good/getList.action"  
						                }  
						            }  
                     }).data("kendoComboBox");	
					...
					var categories = $("#departmentxz").kendoComboBox({  
			            filter: "contains",  
			            placeholder: "请选择",  
			            ...
			            dataSource: {  
			                type: "json",  
			                transport: {  
			                    read: "${pageContext.request.contextPath }/process/gettodoTask.action"  
			                }  
			            }  
        			 }).data("kendoComboBox");	
					...
					$("form").submit(function(){
						...
						$("#detailslist tbody tr").each(function(index, item) {
							var goodname =  $(this).find('td').eq(1).find("input").val().trim();
							var goodmodel = $(this).find('td').eq(2).find("input").val().trim();
							var goodproducer = $(this).find('td').eq(3).find("input").val().trim();
							var unit = $(this).find('td').eq(4).find("input").val().trim();
							var stock = $(this).find('td').eq(5).find("input").val().trim();
							jsonstr+="{\"" + "goodname" + "\":\""+ goodname + "\"," + 
								"\"" + "goodmodel" + "\":\""+ goodmodel + "\"," + 
			...
			function goodproducer(o,e){
				$.ajax({
			          url: "${pageContext.request.contextPath }/good/getStock.action",
			          type: "POST",
			          dataType: "json",
			          data: {
			            "goodname": $("#goodname" + e + '').val(),
			            "goodmodel": $("#goodmodel" + e + '').val(),
			            "goodproducer": $("#goodproducer" + e + '').val()
			          },
			          async: false,
			...
			function addOne(){
				...
				'<td  ><span class="span1" id="num" >1</span>'+
				'</td>'+
				'<td ><input required id="goodname'+index+'" onchange="model(this,' + index + ')" placeholder="物品名称" style="width: 100%;" /></td>'+
				'<td ><input required id="goodmodel'+index+'" placeholder="物品型号" style="width: 100%;" /></td>'+
				'<td ><input id="goodproducer'+index+'" onchange="goodproducer(this,' + index + ')" placeholder="生产商" style="width: 100%;" /></td>'+
				'<td ><input id="unit'+index+'" placeholder="单位" style="width: 100%;" /></td>'+
				'<td >'+
				'	<p>'+
				'		<input id="number'+index+'" type="number" class="am-form-field am-radius"'+
 ...
 */
	...
	function sfmzkc(o,e){
		...
		var name = $(td_content.eq(8)).find("input[id='yjz"+e+"']").val();
		if(o.value > name){
			alert("库存不足，当前库存数量为：" + warning);
			o.val('');
		}
	}
...
'<td ><input id="goodproducer" placeholder="生产商" class="am-form-field am-radius" readonly="readonly" /></td>'+
'<td ><input id="unit" placeholder="单位" class="am-form-field am-radius" readonly="readonly" /></td>'+
'<td >'+
'	<p>'+
'		<input id="price" type="number" class="am-form-field am-radius"'+
'			placeholder="单价" readonly="readonly" />'+
'	</p>'+
'</td>'+
'<td >'+
'	<p>'+
'		<input id="number" onchange="sfmzkc(this,' + index + ')" type="number" class="am-form-field am-radius"'+
'			placeholder="数量" required min="1" />'+
'	</p>'+
'</td>'+
...
<body>
    ...
    <div class="listbiaoti am-cf">
      <ul><img style="margin:-5px 5px 0px 0px;" src="<c:url value='/img/index/menu6.png'/>">商品出库</ul>
    ...
    <div class="fbneirong">
      <form id="myform" action="${pageContext.request.contextPath }/good/outdepod.action" method="post">
      <input id="text" type="hidden" name="textjosn" value="">
      	<table class="am-table am-table-bordered">
      		<tr>
      			<td width="10%">领用人员</td>
      			<td width="40%"><input type="text" id="responsible" class="am-form-field am-radius" name="responsible" required/></td>
      		<!-- </tr>
      		<tr> -->
      			<td width="10%">使用部门</td>
      			<td width="40%"><!-- <input type="text" class="am-form-field am-radius" name="usedepartment" /> -->
      			<input id="departmentxz" placeholder="请选择部门" onchange="sybm(this)" style="width: 100%;"  name="usedepartment" required/> 
      			<input id="zddepartmentxz" style="height:1px; width: 100%;opacity:0; margin-top: -5px;" required onfocus="gbjr(this)" ></td>
      		</tr>
      		<%-- <tr>
      			<td>出库时间</td>
      			...
      			<td  colspan="3">
      				<table style="margin-bottom: 0px;" class="am-table am-table-bordered" id="detailslist">
      						<thead>
									...
									<tr class="am-success">
										<th class="table-id" style="text-align: center;width: 20px;">序号</th>
										...
										<th class="table-type" style="text-align: center;width: 20px;">生产厂家</th>
										...
										<th class="table-type" style="text-align: center;width: 40px;">操作</th>
									</tr>
								</thead>
								<tbody id="apply-table">
									<tr  id="detail" style="background-color: white;">
										...
										<td >
 											<input id="goodmodel1" placeholder="物品型号" style="width: 100%;" required/> 
										</td>
										<td >
 											<input id="goodproducer1" onchange="goodproducer(this,1)" placeholder="生产商" style="width: 100%;" /> 
										</td>
										<td >
 											<input id="unit1" placeholder="单位" style="width: 100%;" /> 
										</td>
										<td>
												<input  id="number1" type="number" class="am-form-field am-radius"
													placeholder="数量" required min="1" />
										</td>
										<td ><a type="button"
											class="am-btn am-btn-default am-btn-xs am-text-danger am-round "
										...
										<td >
 											<input id="goodproducer" placeholder="生产商" class="am-form-field am-radius" readonly="readonly" /> 
										</td>
										<td >
 											<input id="unit" placeholder="单位" class="am-form-field am-radius" readonly="readonly" /> 
										</td>
										<td>
												<input id="price" type="number" class="am-form-field am-radius"
													placeholder="单价"step="0.01" readonly="readonly" />
										</td>
										<td>
												<input id="number" onchange="sfmzkc(this,1)" type="number" class="am-form-field am-radius"
													placeholder="数量" required min="1" />
										</td>
										<td style="display: none;">
												<input id="yjz" type="number" class="am-form-field am-radius"
